...
static const char blob_data[] = "This blob is read-only and exported with debugfs_create_blob().\n";
static struct debugfs_blob_wrapper debug_blob = {
    .data = (void *)blob_data,
    .size = sizeof(blob_data) - 1,
};

static ssize_t message_read(struct file *file, char __user *buffer, size_t len, loff_t *ppos)
{
    ssize_t ret;
...
static ssize_t message_write(struct file *file, const char __user *buffer, size_t len, loff_t *ppos)
{
    ssize_t copied;
    ...
    mutex_lock(&message_lock);
    copied = simple_write_to_buffer(message, sizeof(message) - 1, ppos, buffer, len);
    if (copied > 0) {
        message_len = *ppos;
